    ...
    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5, py: 1.25 }}>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography
          ...
          color="text.disabled"
          display="block"
          sx={{ fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.06em', lineHeight: 1.4 }}
        >
          {label}
        </Typography>
...
export default function FarmQuickViewModal({ farm, onClose }: FarmQuickViewModalProps) {
  const navigate = useNavigate();

  const open = Boolean(farm);
      ...
      <Box sx={{ position: 'relative', height: 220, overflow: 'hidden', flexShrink: 0 }}>
        ...
        <Box
          sx={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(to top, rgba(0,20,60,0.82) 0%, rgba(0,20,60,0.15) 55%, transparent 100%)',
          }}
        />
        ...
        <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, p: 2.5 }}>
          ...
          <Typography
            ...
            fontWeight={800}
            color="white"
            sx={{ textShadow: '0 1px 6px rgba(0,0,0,0.5)', lineHeight: 1.2 }}
          >
            {farm?.name}
          </Typography>
        </Box>
        ...
        <DialogContent sx={{ p: 0 }}>
          <Box sx={{ px: 3, py: 2 }}>
            ...
            <StatRow
              ...
              value={`${farm.workerCount} worker${farm.workerCount !== 1 ? 's' : ''}`}
            />
            <Divider />
